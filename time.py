...
# Create a mapping from Product_Id to its weight.
...
def get_daily_target(current_day):
    ...
    # Working days: target between 13000 and 17000.
    # Weekends: target between 17000 and 20000.
    if weekday < 5:
...
for current_day in date_list:
    ...
    day_total_sale = 0
    day_order_totals = []  # Track order total repetitions
    daily_deliveries = 0
    ...
    while order_count < MIN_ORDERS_PER_DAY or day_total_sale < daily_target:
        ...
        # Prevent same order total from repeating > 3 times in one day.
        ...
        # Choose order type and calculate delivery charge if needed.
    ...
    # If after minimum orders the day's total sale is still below target, add extra minimal orders.
    while day_total_sale < daily_target:
        order_id = next_order_id()
        prod = product_df.loc[product_df["Price"].idxmin()]  # minimal cost product
        quantity = 1
...
print("Main CSV files generated:")
...
# 5a. Peak Hours Table
# Convert the Time column to hour (integer).
orders_df["Hour"] = pd.to_datetime(orders_df["Time"], format="%H:%M").dt.hour
peak_hours_df = orders_df.groupby("Hour").size().reset_index(name="Order_Count")
peak_hours_df.to_csv("cafe_peak_hours.csv", index=False)

# 5b. Order Type Summary Table
# Create a pivot that shows count of each Order_Type per day.
order_type_summary_df = orders_df.groupby(["Date", "Order_Type"]).size().unstack(fill_value=0)
order_type_summary_df.reset_index(inplace=True)
order_type_summary_df.to_csv("cafe_order_type_summary.csv", index=False)

print("Additional CSV files generated:")
print(" - Peak Hours: cafe_peak_hours.csv")
print(" - Order Type Summary: cafe_order_type_summary.csv")
